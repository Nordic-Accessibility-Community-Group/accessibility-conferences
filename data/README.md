...
4. For an event with specific times, add `start_time`, `end_time` and the IANA `time_zone`. Omit all three for an all-day event.
5. Set `attendance.onsite` and `attendance.online` independently. At least one must be `true`.
6. For onsite events, add the two-letter ISO `country_code` in quotation marks, such as `"FI"` or `"US"`.
